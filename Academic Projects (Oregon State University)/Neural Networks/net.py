...
number_of_layers = 6
width_of_layers = 256 # only matters if number of layers > 1
activation = "ReLU" 
...
def main():
  ...
  predictions = predict_test_set(net, X_test)  # Assuming net is your trained model
...
class LinearLayer:
  ...
  def backward(self, grad):
    # grad is dL/dZ
    self.grad_weights = self.input.T @ grad  # Compute dL/dW as in Eq. 32
    self.grad_bias = np.sum(grad, axis=0, keepdims=True)  # Compute dL/db as in Eq. 27
    grad_input = grad @ self.weights.T  # Compute dL/dX as in Eq. 35
    return grad_input
...
def loadData(normalize = True):
  train = np.loadtxt("/Users/sankethkaruturi/Downloads/cs434-f23-hw3/mnist_small_train.csv", delimiter=",", dtype=np.float64)
  val = np.loadtxt("/Users/sankethkaruturi/Downloads/cs434-f23-hw3/mnist_small_val.csv", delimiter=",", dtype=np.float64)
  test = np.loadtxt("/Users/sankethkaruturi/Downloads/cs434-f23-hw3/mnist_small_test.csv", delimiter=",", dtype=np.float64)

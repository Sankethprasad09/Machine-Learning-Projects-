...
def main():
    ...
    for k in [1,3,5,7,9,99,999,8000]:
        ...
        t1 = time.time()
        print("k = {:5d} -- train acc = {:.2f}%  val acc = {:.2f}% (+/- {:.4f})\t\t[exe_time = {:.2f} seconds]".format(
...
def get_nearest_neighbors(example_set, query, k):
    # Compute the differences between each row in the example set and the query
    differences = example_set - query
    # Compute the Euclidean distances
    distances = np.linalg.norm(differences, axis=1)
    # Get the indices of the k smallest distances
    idx_of_nearest = np.argsort(distances)[:k]
    
    return list(idx_of_nearest)
...
def knn_classify_point(examples_X, examples_y, query, k):
    ...
    # Fetch the labels of these nearest neighbors
    nearest_labels = examples_y[nearest_indices]
    # Get the majority label as the prediction
    predicted_label = np.bincount(nearest_labels.astype(int).flatten()).argmax()
...
def cross_validation(train_X, train_y, num_folds=4, k=1):
    # Split the dataset into num_folds chunks
    chunk_size = len(train_X) // num_folds
    accuracies = []
    ...
    for i in range(num_folds):
        ...
        # Create validation and training splits for the current fold
        val_X = train_X[start_idx:end_idx]
        val_y = train_y[start_idx:end_idx]
        
        train_fold_X = np.vstack([train_X[:start_idx], train_X[end_idx:]])
        train_fold_y = np.vstack([train_y[:start_idx], train_y[end_idx:]])
        
        # Predict on validation set
        pred_y = predict(train_fold_X, train_fold_y, val_X, k)
        # Calculate accuracy
        accuracies.append(compute_accuracy(val_y, pred_y))
...
# Load data
def load_data():
    traindata = np.genfromtxt('/Users/sankethkaruturi/Downloads/train.csv', delimiter=',')[1:, 1:]
    train_X = traindata[:, :-1]
    train_y = traindata[:, -1]
    train_y = train_y[:,np.newaxis]
    
    test_X = np.genfromtxt('/Users/sankethkaruturi/Downloads/test_pub.csv', delimiter=',')[1:, 1:]
